...
public class Methods {
	...
	public static String readableBrgOrCrs(String brgOrCrs) {
		while (brgOrCrs.length() < (brgOrCrs.contains(".") ? 5 : 3))
			brgOrCrs = "0" + brgOrCrs;
	...
	public static String applyYorKorNM(String range) {
		...
		if (range.endsWith("y") || range.endsWith("yds")
				|| range.endsWith("yards")) {
	...
	public static double convertFromBearingToDegrees(double bearing) {
		...
		// keeps the angle we're working with within 360*
		bearing = posifyAngle(bearing);

		if (bearing == 0 || bearing == 360)
			return 90;
		else if (bearing <= 90) // Quadrant 1
			return 90 - bearing;
		else if (bearing <= 180) // Quadrant 2
			return 90 + 360 - bearing;
		else if (bearing <= 270) // Quadrant 3
	...
	public static double convertFromDegreesToBearing(double decimalDegree) {
		...
		if (decimalDegree == 90)
			return 0;
		else if (decimalDegree <= 90) // QuadecimalDegreerant 1
			return 90 - decimalDegree;
		else if (decimalDegree <= 180) // QuadecimalDegreerant 2
			return 360 - (decimalDegree - 90);
		else if (decimalDegree <= 270) // QuadecimalDegreerant 3
			return 180 + (270 - decimalDegree);
		else
			// QuadecimalDegreerant 4
			return 180 - (decimalDegree - 270);
	}
	...
	public static Location getDestinationLocation(Location location,
			double course, double distanceOrRange) {
		double d = distanceOrRange;

		double dx = Math.round(Math.cos(Math
				.toRadians(convertFromBearingToDegrees(course)))
				* d);
		double dy = Math.round(Math.sin(Math
				.toRadians(convertFromBearingToDegrees(course)))
				* d);
	 ...
	 */
	public static Location getDestinationLocation(Location location,
			double course, int speed, int time) {
	...
	public static double getBearing(Combatant combatant1, Combatant combatant2) {
		...
		// accounts for cardinal directions
		if (dx == 0 && dy == 0)
			return 0;
		else if (dx == 0) {
			if (dy > 0)
			...
			// gets reference angle
			double refAngle = Math.abs(Math.toDegrees(Math.atan(dy / dx)));

			// applies offset of reference angle based on quadrant
			if (dy < 0 && dx < 0) {
				angle = refAngle + 180;
			} else if (dx < 0) {
		...
		// converts angle into bearing and returns
	...
	public static boolean isNumber(String str) {
		int len = str.length();
		int chs = 0; // counter for number of .'s
	...
	// /////////////////////////////////////////////////////////////////////////
	// STRING AND COMMAND PARSING METHODS \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
	// /////////////////////////////////////////////////////////////////////////
	 ...
	 */
	public static String replaceAllArgsWith(String str, String[] args,
			String replacement) {
		int cnt = args.length;

		if (str.length() < 1 || cnt < 1)
			return str;
	...
	public static void runTests() {
		...
		// TEST degree and bearing conversion
		System.out.println(" ^^^ TEST: Degree and Bearing Conversion");

		boolean worked = true;
		for (double i = 0; i < 360; i++) {
			// System.out.print("[i=" + i + " deg ->" +
			// Methods.convertFromDegreesToBearing(i) + "] ");
			// System.out.println(" [i=" + i + " crs ->" +
			// Methods.convertFromBearingToDegrees(i) + "]\n");
			worked &= (Methods.convertFromDegreesToBearing(Methods
					.convertFromBearingToDegrees(i)) == i)
					&& (Methods.convertFromBearingToDegrees(Methods
							.convertFromDegreesToBearing(i)) == i);
		}

		System.out.println("Degree and Bearing conversion "
				+ (worked ? "works!" : "does NOT work!"));

		// TEST parseCommandParameters(ArrayList<String> parameters)
		/*
		 * System.out.println(" ^^^ TEST: parseCommandParameters(...)");
		 * 
		 * ArrayList<String> parameters = new ArrayList<String>();
		 * parameters.add("name"); parameters.add("age"); Object[] inputs =
		 * Methods.parseCommandParameters(parameters); for (int i = 0; i <
		 * inputs.length; i++) System.out.println(inputs[i]);
		 */

		// TEST stripLeadingZeros(String str)
